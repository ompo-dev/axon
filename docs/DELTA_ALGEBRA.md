...
`SumState` é imutável: `apply_delta`, `apply_coalesced` e `full_after` retornam novo estado. Testes exigem igualdade exata entre os três caminhos e rejeitam suporte ou índice inválido.

`coalesce_adjacent_last_writes` remove somente escritas consecutivas para mesma chave. Isso preserva estado final; não é uma permissão para reordenar eventos com efeitos intermediários observáveis.
